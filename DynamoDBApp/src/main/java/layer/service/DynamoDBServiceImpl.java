...
public class DynamoDBServiceImpl implements DynamoDBService {
    ...
    public String updateUser(Map<String, String> pathParameters, String inputBody) {
        ...
        if (existingUser != null) {
            RequestBody requestBody = new Gson().fromJson(inputBody, RequestBody.class);
            if (requestBody.getName() != null) {
                existingUser.setName(requestBody.getName());
            }
            if (requestBody.getLocation() != null) {
                existingUser.setLocation(requestBody.getLocation());
            }
            if (requestBody.getBirthday() != null) {
                existingUser.setBirthday(requestBody.getBirthday());
            }

        dynamoDBMapper.save(existingUser);
        return getJsonResponse("User updated successfully: " + existingUser.getEmail());
        } else {
            return getJsonResponse("Such user not found");
        }
    } catch (Exception e) {
